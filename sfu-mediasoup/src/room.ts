...
export class Room implements ErrorHandler {
  ...
  private _routers: Map<string, { num: number; router: Router }>;
  ...
  private async createRouterHandler({ ws, data, response }: Handler) {
    ...
    try {
      ...
      if (!this._routers.has(ok_router.id)) {
        this._routers.set(ok_router.id, { num: 1, router: ok_router });
      } else {
        const r = this._routers.get(ok_router.id)!;
        r.num++;
      }
  ...
  private getRouterRtpCapabilitiesHandler({ ws, data, response }: Handler) {
    try {
      ...
      response({
        data: {
          mediaCodecs: this._routers.get(data.router_id)!.router
            .rtpCapabilities,
        },
      });
    } catch (error) {}
    ...
    try {
      ...
      response({
        data: {
          ...
          transportType:
            transport.appData.consuming === true ? "consuming" : "producing",
        },
      });
    } catch (e: any) {
  ...
  private async connectWebRTCTranportHandler({ ws, data, response }: Handler) {
    try {
      ...
      this.log.info(
        "[WebRTCTransport-Connect-Event] Transport [%s] is connecting.",
        transport?.id
      );
  ...
  private async createProduceHandler({ ws, data, response }: Handler) {
    const { router_id, transport_id, rtpParameters, rtpCapabilities, kind } =
      data;
    const transport = this._transports.get(transport_id);
    ...
    this.log.info(
      "[CreateProducer-Event]：Create Producer [%s] with Router [%s]",
      producer.id,
      router_id
    );
    ...
    producer.on("transportclose", () => {
      this.log.info(
        "[CloseProducer-Event]：Producer [%s] is closed because transport closed",
        producer.id
      );
    ...
    if (this._routers.has(router_id)) {
      router = this._routers.get(router_id)!.router;
    } else {
    ...
    if (this._pipeTransportsRouter !== undefined) {
      ...
      await this._connectToOtherRouter(
        router!,
        this._pipeTransportsRouter,
        producer.id
      );
      for (let [key, value] of this._serverAndPipeTransport) {
        ...
        if (this._pipeTransports.has(value)) {
          ...
          const consumer = await this.createPipeTransportConsume(
            pt,
            producer.id,
            rtpCapabilities
          );
  ...
  private _getOtherRouters({
    excludeRouterId = "",
  }: {
    excludeRouterId: string;
  }): Array<Router> {
    let routerList: Array<Router> = [];
    this._routers.forEach((routerObj) => {
      if (routerObj.router.id !== excludeRouterId) {
        routerList.push(routerObj.router);
      }
    });
  ...
  private async createPipeTransportHandler({ ws, data, response }: Handler) {
    ...
    if (this._pipeTransportsRouter === undefined) {
      const worker = this._listener.pipeTransportWorker!;
      this._pipeTransportsRouter = await worker.createRouter({ mediaCodecs });
      this.log.info(
        "[CreatePipeTransportRouter-Event]：Craete new PipeTransportRouter [%s]",
        this._pipeTransportsRouter.id
      );
    } else {
      this.log.warn(
        "[CreatePipeTransportRouter-Event]：PipeTransportRouter is already exist!"
      );
    }
    ...
    if (state) {
      this.log.warn(
        "[CreatePipeTransport-Event]：IP [%s] already has pipetransport [%s]",
        server_id,
        pipeTransport.id
      );
    } else {
      this.log.info(
        "[CreatePipeTransport-Event]：Room [%s] Create Pipetransport [%s] to connect ip [%s]",
        this._id,
        pipeTransport.id,
        server_id
      );
  ...
  private async createPipeTransportProduceHandler({
    ws,
    data,
    response,
  }: Handler) {
    ...
    if (this._pipeTransports.has(pipeTransportId)) {
      ...
      for (let { producer_id, kind, rtpParameters } of consumerMap) {
        ...
        this._pipeTransportProducers.set(producer.id, producer);
        // TEST TO DEBUG
        ...
        // });
        console.log(
          "[CreateProducer-PipeTransport-Event]：PipeTransport [%s] create Producer [%s]",
          pt.id,
          producer.id
        );
        for (let [key, routerObj] of this._routers) {
          await this._connectToOtherRouter(
            this._pipeTransportsRouter!,
            routerObj.router,
            producer!.id
  ...
  private async createPipeTransportConsumeHandler({
    ws,
    data,
    response,
  }: Handler) {
    ...
    if (this._pipeTransports.has(pipeTransportId)) {
      ...
      for (let [routerId, producerInfoArray] of Object.entries(producerMap)) {
        const router = this._routers.get(routerId)!.router;
        for (let {
          producerId,
          rtpCapabilities,
        } of producerInfoArray as Array<any>) {
          if (this._pipeTransportsRouter !== undefined) {
  ...
  private async createPipeTransportConsume(
    pt: PipeTransport,
    producerId: string,
    rtpCapabilities: any
  ) {
    if (
      !this._pipeTransportsRouter?.canConsume({ producerId, rtpCapabilities })
    ) {
  ...
  private async closeTransportHandler({ ws, data, response }: Handler) {
    ...
    if (this._transports.has(id)) {
      ...
      this.log.info(
        "[WebRTCTransport-CloseTransport-Event]：Close transport [%s]",
        transport?.id
      );
      transport?.close();
