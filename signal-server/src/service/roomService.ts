  ...
  private _registerRouterHandler() {
    ...
    this._router.get("/getRoomList", async (req: Request, res: Response) => {
      try {
        ...
        const promiseList = temp_list.map(
          (values: { id: string; name: string }) => {
            return new Promise<void>(async (resolve, reject) => {
              const roomDataList =
                await this._roomController.getAllRoomPlayerList(values.id);
              roomList.push({
                roomId: values.id,
    ...
    this._router.post("/createRoom", async (req: Request, res: Response) => {
      try {
        ...
        const r = await this._roomCreator.createRoom(
          id,
          room_id,
          req.body.room_name
        );
